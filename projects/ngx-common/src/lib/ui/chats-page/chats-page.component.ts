...
const SELECTED_KEY = 'chats.selectedId';
...
@Component({
  ...
  template: `
    <app-page-layout class="flex h-full flex-col" expandedWidth="w-96" storageKey="layout.chats" panelTitle="Chats">
            ...
            >
              ...
              <div class="min-w-0 flex-1">
                <div class="flex items-center gap-1.5">
                  <lucide-icon name="message-square" [size]="14" class="shrink-0 text-primary-600" />
                  <p class="truncate text-sm font-medium text-text">{{ conv.title }}</p>
                </div>
                <div class="mt-1">
                  <span class="text-xs font-medium text-text-muted">{{ conv.llm }} / {{ conv.model }}</span>
                </div>
              </div>
            </button>
          }
        ...
        @if (!hasChild()) {
          <div class="flex min-h-full flex-col items-center justify-center gap-4 p-8">
            ...
            <div class="text-center">
              <h2 class="mb-1 text-xl font-bold text-text">Select a chat</h2>
...
export class ChatsPageComponent implements OnInit {
  ...
  protected readonly filteredConversations = computed(() =>
    this.conversationService.conversations()
      .filter(c => c.conversation_type === 'chat')
  ...
  private loadConversations(isRefresh: boolean): void {
    ...
    this.conversationService.getConversations().pipe(
      ...
      error: (err) => {
        this.error.set(err instanceof TimeoutError ? 'Request timed out. Please try again.' : 'Failed to load chats.');
        this.loadingConvs.set(false);
        this.refreshing.set(false);
      },
    });
